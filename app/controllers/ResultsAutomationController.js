define(['app'], function (app) {
    app.controller("ResultsAutomationController", function ($scope, $http, $localStorage, $rootScope, $state, $stateParams, $document, AppSettings, $timeout, PreExaminationService) {
            ...
            if ($scope.myFile != '') {
                ...
                reader.onload = function (e) {
                    ...
                    if ($scope.XL_row_object.length > 0) {
                        ...
                        var tempArray = ["ROLL_NO", "CNAME", "MATHS", "PHYSICS", "CHEMISTRY", "BIOLOGY", "MPC_TOTAL", "MBIPC_TOTAL", "CASTE", "MPC_PASSTAG", "MBIPC_PASSTAG"];
        ...
        $scope.GenerateNr = function (Nrscheme) {
            ...
            var loadData1 = PreExaminationService.GenerateNrData($scope.UserName)
        ...
        $scope.RankGeneration = function () {
            $scope.reload = true;
            var loadData1 = PreExaminationService.RankGeneration($scope.UserName)
            loadData1.then(function (res) {
                var data = JSON.parse(res)
                if (data[0].ResponceCode == '200') {
                    $scope.Result = true;
        ...
        $scope.WantingsReport = function () {
            $scope.reload = true;
            var loadData1 = PreExaminationService.WantingsReport($scope.UserName)
            loadData1.then(function (res) {
                var data = JSON.parse(res)
                if (data[0].ResponceCode == '200') {
                    $scope.Result = true;
        ...
        $scope.MarksPosting = function () {
            $scope.reload = true;
            $scope.ResultNotFound = false;
            $scope.SuccessStatus = false;
            $scope.FailStatus = false;

            var loadData1 = PreExaminationService.MarksPosting($scope.UserName)
        ...
        $scope.UploadExcel = function () {
            ...
            $scope.Exceldat.forEach(function (element) {
                ...
                var obj = {
                    "ROLL_NO": element.ROLL_NO, "CNAME": element.CNAME, "MATHS": element.MATHS, "PHYSICS": element.PHYSICS, "CHEMISTRY": element.CHEMISTRY,
                    "BIOLOGY": element.BIOLOGY, "MPC_TOTAL": element.MPC_TOTAL, "MBIPC_TOTAL": element.MBIPC_TOTAL, "CASTE": element.CASTE, "MPC_PASSTAG": element.MPC_PASSTAG, "MBIPC_PASSTAG": element.MBIPC_PASSTAG                }
                $scope.filteredArray.push(obj)
            });
            console.log($scope.filteredArray)
            var uploadJson = PreExaminationService.UploadResultFileJson(JSON.stringify($scope.filteredArray), $scope.UserName);
        ...
        $scope.Download = function () {
            ...
            window.location.href = location + '/contents/sample_data.xlsx';
